...
async def on_request_updates(uid: str, timestamp):
    logger.info(f"Request updates: {timestamp} form {uid}")
    ...
    logger.info(f"Updates for {uid}: {len(new_chats)} chats, {len(updated_chats)} updated, {len(deleted_chats)} "
                f"deleted chats, {len(new_messages)} new messages, {len(deleted_messages)} deleted messages")
    return res


async def on_request_updates_v2(uid: str, timestamp):
    logger.info(f"Request updates (v2): {timestamp} form {uid}")
    timestamp = datetime.fromisoformat(timestamp).replace(tzinfo=None)
    uow = UnitOfWork()
